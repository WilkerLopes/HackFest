  ...
  switch (pageAtual) {
    case 1:
      ...
      if (nomeU != null && nomeU != "" && idadU != null && idadU != "") {
        ...
        $("#msg").text("Preencha todos os campos antes de continuar!");
      }
      break;
    case 2:
      console.log(pageAtual);
...
function printLocales(localPprint) {
  for (let contL = 0; contL <= (localesMaps1.length - 1); contL++) {
    if (contL == 0) {
      $("#tabs1").append("<li class='tab col s3'><a class='active' href='#tab1l" + contL + "'>" + localesMaps1Name[contL] + "</a></li>");
    } else {
      $("#tabs1").append("<li class='tab col s3'><a  href='#tab1l" + contL + "'>" + localesMaps1Name[contL] + "</a></li>");
    };
    $("#printMaps" + localPprint).append("<div id='tab1l" + contL + "' class='col s12 center'><div class='lineInfoTabs'><div class='row'><div class='col s8'><p>" + localesMaps1Desc[contL] + "</p></div><div class='col s4'><span>" + localesMaps1Data[contL] + "</span></div></div></div><div class='mapsView12'> <iframe src='" + localesMaps1[contL] + "' width='" + largura12 + "' height='300' frameborder='0' style='border:0;' allowfullscreen='' aria-hidden='false' tabindex='0'></iframe> </div></div>");
  }
}
//tabs
